...
import com.example.groceryapp.adapters.AdapterPromotionShop;
...
public class PromotionCodesActivity extends AppCompatActivity {
    ...
    private void loadExpirePromoCode(){
        ...
        final String todayDate = day +"/"+month+"/"+year; // e.g 20/09/2020
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        for (DataSnapshot ds: snapshot.getChildren()){
                            ...
                            try {
                                ...
                                if(expDate.compareTo(String.valueOf(currentDate)) > 0){
                                    // date 1 occurs after date 2
                                }
                                else if( expireDate.compareTo(currentDate) < 0){
                                    // date 1 occurs before date 2 (i.e. Expire)
                                    // add to list
                                    promotionArrayList.add(modelPromotion);
                                }
                                else if( expireDate.compareTo(currentDate) == 0){
